...
public sealed class PreviewForm : Form
{
    ...
    ///
    /// **原因は未解明である。** こちらのラスタも送出バイトもインクによらず同一で
    /// あることは確かめてある(1200dpi で黒だけ / 白だけのプレビューは画素単位で一致し、
    /// RGL のバイト数も一致)。当初「特色は Standard モードなので 600dpi に落ちる」と
        ...
        IReadOnlyList<(string Label, bool IsNonProcess, bool Used)> inks)
    {
        ...
        return
            $"⚠ 1200dpi では、次のインクが横幅 2 倍で刷られた実測があります: {string.Join(", ", affected)}" +
            Environment.NewLine +
            "  原因は未解明です。特色を含むジョブは解像度を 600 にしてください(D-051)。";
    }
