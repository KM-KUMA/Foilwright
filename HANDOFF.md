...
**送出方法もシロと確定した(2026-08-03)。** Windows 標準のテストページと自作ツールの RAW 送出を比較したところ、**両者ともまったく同じエラー**(`DetectedErrorState = 1` / `PrinterState = 2`)になった。経路の違いは症状に影響しない。

**残るのは機械側の実行条件。** プリンタは通信に応答し、MD 固有のエラーで拒否している。黒リボン 1 本では解消しなかった。
...
**最優先: 機械の実行条件を整える。** 送出側で打てる手は尽きた(実装は golden と一致、送出経路も Windows 標準と同一結果)。

**判定手順が確立している。** 送出直後に `DetectedErrorState` を読めば受理か拒否かが分かる。**紙を消費せずに判定できる**ので、条件を変えながら試せる。

```powershell
Get-CimInstance Win32_Printer -Filter "Name='Foilwright-MD5500'" |
  Select-Object DetectedErrorState, PrinterState
```

`DetectedErrorState` は **送出直後の値だけが意味を持つ**(Windows は印刷時にしか問い合わせない)。`2` = 受理、`1`(Other) = MD 固有のエラーで拒否。

1. **カートリッジを 4 本揃えて試す。** MD-5000 では 1 本のとき無反応、4 本にすると挙動が変わった。公式ドライバも装着構成を確認している(§13.5.6)
2. **カートリッジの認識を疑う。** バーコードが読めているか、装着が確実か、リボン自体が正常か(§10.9.2)
3. **異音の原因を特定する。** MD-5500 は未解消。§10.9.5 の外部事例では紙押さえスプリングの曲がりだった
4. **公式ドライバの出力バイト列を手に入れる。** 仮想 PC のポート追加一覧に ALPS 系のポートモニタがあるか確認し、あれば `MD_LPT1:` を作って FILE: 出力を再試行。なければドライバ一式の再インストール
