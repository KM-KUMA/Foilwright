...
def apply_opaque_white_mode(
    ...
    There is no JobAssembly-equivalent layer in ref/ (unlike
    src/Foilwright.Core/JobAssembly.cs, which owns white-mode selection
    for the C# port); D-027's white-mode selector (none/auto/magic) was
    never built out here, only the underlying `auto_undercoat` flag that
    to_planes_magic/to_planes_auto honour. This function mirrors
    JobAssembly.ApplyOpaqueWhite/ComputeNonWhitePixelPlane instead of
    extending to_planes_magic/to_planes_auto themselves, so their
    golden-verified byte output (DOMAIN.md §0) is untouched.
    ...
    white-mode target is undefined, same as JobAssembly's rule.
    ...
    Returns a new dict; `planes` itself is not mutated.
    """
    undercoat_names = [ink["name"] for ink in inks if ink.get("auto_undercoat")]
    if len(undercoat_names) != 1:
        return planes
